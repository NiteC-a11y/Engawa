...
- `docs/adr/`（0001〜0026）, `docs/TECH_RULES.md`, `docs/Backlog.md`
...
**既知の宿題（Open Questions）**: 長命セッションの compaction/fork 閾値（＝長時間稼働で茶々の出力に注入プロンプトの復唱＋地の思考が染み出す不具合。表示ガード `strip_resident_leak`＋`/restart` で対症済み・**根治=Nターン自動リフレッシュ/染み出し検知→自動再生成は未実装**・Backlog）／ `/codex <自由テキスト>` のプロンプトインジェクション（配布時のみ・検討メモは Backlog）／ 客人の作り込み度 ／「茶々が反応しない（……）」の UI 表現。

---

## 参照
- 設計判断と却下理由 → **docs/adr/README.md**（0001〜0026）
