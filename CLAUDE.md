...
- **【技術的負債】** node 取り残し刈り（Job Object 化は残） / **ACP timeout・握手 teardown（codexレビュー S1/S2 実装済み・残=cancel後の短 bounded wait）** / SQLite 永続化 / 天気座標の大阪固定→設定化 / 茶々用 CLAUDE.md の別ディレクトリ運用 / 体感ナレーション層。
