...
- `engawa.json` + `config.py` — アプリ挙動の設定（model/guest/間合い/topic）。優先順位 **env(ENGAWA_*) > engawa.json > 既定**。キーは入れない(adr/0002)。
...
- **スラッシュ**: `/codex <人格>`（客人召喚）/ `/blackjack [見る]`（AIとブラックジャック・「見る」で観戦＝全AI・要 `pip install rlcard`・ADR-0017）/ `/arc [雀|猫|風]`（箱庭再生・デバッグ）/ `/model`（今のモデル表示・住人/客人）/ `/help` / `/quit`
