...
- `assets/props.json` + `assets/props/` — **縁側の小物の台帳と絵**（adr/0032・7/18）。何を・どこに・いつ（months=月ゲート＝実日付の季節反応・蚊取り線香は6〜9月）・どんな演出（汎用語彙 `rise`＝煙/湯気の粒・**小物専用コード禁止＝新小物はコード0行で台帳に1行**）を台帳が持ち、`src/props.py`（純関数）＋`views._load_props` が描く（昼夜膜の下＝夜は小物も暮れる・欠損スキップ）。台帳は **renderer 非依存の契約**＝将来の canvas シーン化（庭側ビュー最終形・Backlog）に台帳ごと引っ越す。差し替えは `ENGAWA_PROPS_CONFIG`/`engawa.json[assets].props_config`。位置調整はユーザー目視（left_pct/bottom_pct）。
...
- `docs/adr/`（0001〜0032）, `docs/TECH_RULES.md`（**固有の実装契約のみ**＝ワイヤ契約/OS地雷/境界/テスト運用・決定は1行＋ADRポインタ・adr/0030）, `docs/Backlog.md`
...
## 参照
- 設計判断と却下理由 → **docs/adr/README.md**（0001〜0032）
