...
- **【技術的負債】** node 取り残し刈り（Job Object 化は残） / **ACP timeout・握手 teardown（codexレビュー S1/S2 実装済み・cancel後の bounded wait も実装6/29）** / SQLite 永続化 / ~~天気座標の大阪固定~~→**設定化済み**（`ENGAWA_WEATHER_LAT/_LON/_WEATHER_TZ`＋`ENGAWA_PLACE_LABEL`・`engawa.json[weather]`・既定は大阪で現行維持・`sources._weather_url`／会話・昼夜tintの時刻はローカル依存で別問題として残る） / 茶々用 CLAUDE.md の別ディレクトリ運用 / 体感ナレーション層。
