...
自分のマシンの **Claude / ChatGPT のサブスク認証**で動きます（各自 BYO サブスク＝自分のログインで動かす・API 従量課金はしない設計）。配布は BYO 前提で自由ですが、1つのサブスクを他人のために使う共有サービス化や claude.ai ログインの同梱は各社 ToS 上できません。
...
| Node.js | `npx` 経由で ACP アダプタを起動（`@agentclientprotocol/claude-agent-acp` / `@agentclientprotocol/codex-acp`） |
...
（茶々を**ダブルクリック**すると「ニャー」と鳴いてハート♥がふわっと舞います。撫でるみたいな小ネタ。）
