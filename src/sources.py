...
def place_label():
    """茶々が「〜は晴れ」と言う地名（優先: env/json の `ENGAWA_PLACE_LABEL` > voice culture > locales 既定 大阪
    ＝ADR-0033 追補17。設定が空の時だけ culture へ落ちる＝既定 voice は現行の大阪のまま）。"""
    cfg = config.get_str("ENGAWA_PLACE_LABEL", "weather", "place", "")
    return cfg or voice.place_label()
...
def _persona_matches(guest, tag, guest_id=None):
    """トピックの persona タグが客人の役にマッチするか。タグ無し(None/空)は全員可（graceful degrade
    ＝季節ネタは誰にでも）。一致は2系統: ①タグ＝**安定 id**（"peddler" 等・display の翻訳で壊れない・
    ADR-0033 追補17） ②タグが**役名(display)の一部**に含まれる（"行商"⊂「…行商人」・JP 互換・7/1 修正の維持）。"""
    if not tag:
        return True
    tags = [tag] if isinstance(tag, str) else tag
    guest = guest or ""
    return any((guest_id is not None and str(t) == guest_id) or str(t) in guest for t in tags)


def pick_topic_text(pool, persona, avoid=(), persona_id=None):
    """世間話の“種”を1つ選ぶ（人格マッチ→直近回避→ランダム）。無ければ None。
    確率ゲート(TOPIC_PROB)も履歴も持たない＝純関数（呼び側＝scheduler が握る・ADR-0014 の
    部屋経路復活）。人格マッチは _persona_matches（安定 id 一致 or タグが役名の一部・タグ無しは全員）。"""
    if not pool:
        return None
    matched = [t for t in pool if _persona_matches(persona, t.get("persona"), persona_id)]
...
# ── 客人源（ADR-0008/0015）───────────────────────────────────
# 自発来訪の役プールは culture へ移設（voice.guest_personas()＝id/display・ADR-0033 Inc4）。
# 召喚（/codex <人格>）はユーザー自由入力＝display のみで id 無し。
...
class GuestSource(EventSource):
    ...
    def __init__(self, persona=None, spawn_codex=None):
        self.persona = persona                   # 表示・prompt 注入用の役名（display）
        self.persona_id = None                   # 安定識別子（自発の抽選時のみ・topic 照合用・ADR-0033 Inc4）
    ...
    def reset(self):
        if self._autonomous:
            p = random.choice(voice.guest_personas())      # 来訪ごとに役を着せ替え（culture 解決済みプール）
            self.persona, self.persona_id = p["display"], p["id"]
