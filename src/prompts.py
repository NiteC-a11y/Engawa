...
_lang_note = voice.lang_note   # 本体は voice.py へ移設（sources の narration ビルダーとも共用・7/19 のソロ経路穴の教訓）
...
# barge-in はつぶやき中とは限らない（返答生成中に被せた時も speaking）＝「つぶやき」と断定しない中立表現
_INTERRUPTED_FRAME = "あなたは何か言いかけていたのを止めて、こちらを向いたところ。"


def user_narration(text, ctx=None, interrupted=False):
    # 天気を ctx から渡す（起動直後でも茶々が天気を捏造しないように・Backlog）。
    # ただし「聞かれてないのに天気を言い立てない」よう持たせるだけ。ctx 無しは時刻のみ。
    # interrupted＝喋りかけを cancel された barge-in。UI の「[茶々がこちらを向いた]」は View だけの
    # 演出でセッションに流れない＝振り向いた事実を本人も知るよう、世界の事実として注入に語る。
