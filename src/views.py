...
def _localize_html(html):
    ...
    pairs = (
        ('<button class="ac sel" data-p="">茶々</button>',
         '<button class="ac sel" data-p="">' + voice.loc("ui_chip_resident") + "</button>"),
        ('<button class="ac" data-p="guest">客人</button>',
         '<button class="ac" data-p="guest">' + voice.loc("ui_chip_guest") + "</button>"),
        ('<button class="ac" data-p="both">二人とも</button>',
         '<button class="ac" data-p="both">' + voice.loc("ui_chip_both") + "</button>"),
        ('<button id="send">送信</button>',
         '<button id="send">' + voice.loc("ui_send") + "</button>"),
        ('placeholder="話しかける…"',
         'placeholder="' + voice.loc("ui_placeholder") + '"'),
        ("'話しかける…（Enterで送信・Shift+Enterで改行）'",
         json.dumps(voice.loc("ui_placeholder_send"),
                    ensure_ascii=False)),
        ("'話しかける…（改行OK・送信は Ctrl+Enter か 送信ボタン）'",
         json.dumps(voice.loc("ui_placeholder_newline"),
                    ensure_ascii=False)),
        ('title="閉じる"', 'title="' + voice.loc("ui_close") + '"'),
        ('<div id="nya">ニャー</div>', '<div id="nya">' + voice.loc("ui_meow") + "</div>"),
        ("あなた ›", voice.loc("ui_you") + " ›"),
