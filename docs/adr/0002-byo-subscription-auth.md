# ADR-0002: サブスク認証を継承し、APIキーは使わない（各自 BYO サブスク）
...
## 決定
各アダプタの子プロセスにサブスク認証を継承させ、**`ANTHROPIC_API_KEY` は子プロセスから除去**する。アカウント取り違え防止に **`CLAUDE_CONFIG_DIR` で個人プロファイルを固定できるようにする**（既定は継承・ユーザが明示した時だけ固定＝opt-in。理由は下の制約と実装メモ）。利用は**各自の BYO サブスク**（各ユーザが自分の Claude / ChatGPT ログインで動かす）とする＝ソース/exe の配布は BYO 前提で可。ただし**1つのサブスクを他人のために使う共有サービス化、および claude.ai ログインの製品同梱は ToS 上不可**（下の制約）。
...
- **文言メモ（2026-07-11）**: 当初「個人利用限定」と書いたが、exe 公開後は言葉足らずだった。制約の実体は「**認証モデル**」＝各自の BYO サブスクで動かす／単一サブスクの共有サービス化・claude.ai ログイン同梱が ToS 上不可、であって「配布禁止」ではない（BYO 配布は可）。タイトル・決定・TECH_RULES §3・README(日英)・CLAUDE.md を「各自 BYO サブスク」表現へ統一。
