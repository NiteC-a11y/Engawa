...
- 実機 E2E（ユーザー）→ **済（7/18・範囲限定）**: 代打中の被せ＝guest cancel と直後の次手を実機確認・
  codex-acp の session/cancel は初回観測で問題なし。**未検証のまま＝挨拶/辞去の窓（同日の追記修正で
  中断不可＝cancel 対象外になった）と Responding 中の連打（スコープL・未実装）**。
