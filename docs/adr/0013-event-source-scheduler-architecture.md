...
>
> **構造ドリフト注記（2026-07-11）**: 下図とデータフローは **2026-06-27 時点の目標構成**。以降のリファクタで一部が移動している——(1) `Scheduler.active`（単一）は **ADR-0029 P2** で `active_source`（アーク）/`active_guest`（来訪）に**分割**、図の `_advance()`/`_interrupt()` は実在せず（`_tick`/`on_user_input` へ）。(2) 来訪の codex 往復は **ADR-0015** で `conversation.Room` へ移り、`GuestSource` は `next_phase` を持たず（基底の None を継承）`ensure_agent`/`close` を公開＝「重いアーク」から「room が使う agent 供給源」へ。(3) room の茶々/客人 Speaker と種プールは **ADR-0029 P4a** で `RoomSpeakerFactory`（`room_speakers.py`）へ。**①②③の継ぎ目の思想は不変**で、担い手が controller 群へ再配置されただけ。図は歴史的な目標構成として温存。
