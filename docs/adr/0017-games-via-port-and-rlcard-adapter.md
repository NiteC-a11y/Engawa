...
- **「ゲームモードのまま固まらない」保証**: ①観戦窓×は窓を閉じるだけでなく入力チャネル経由で scheduler に伝え、**お開き（縁側へ復帰）**にする（窓だけ閉じて `Scheduler.game` が残るのを防ぐ）②`_tick` の game ブロックは timeout 以外の例外でも**お開き**にして tick ループを殺さない ③対局中は `/codex`・`/game` をガード（room と game の同時成立を防ぐ）。終了処理は共通 `_teardown_game` に集約。
