...
- ステータス: Accepted（**Inc1〜4 実装済み**・blackjack はフル札描画／uno・leduc は汎用表示・対局運用は `GameController` に集約＝ADR-0029 P3）
- 日付: 2026-06-28
...
### 対局のライフサイクルと入口（2026-06-28〜29・追記）
- **A方式＝基本は 私＋茶々**（観戦は茶々のみ）で**客人 codex は呼ばない**。ゲームが人数を要求する時だけ客人で埋め、終局で破棄。手番プロンプトに自分のスロットを明示（全員の手札が見える blackjack 対策）。
- **起動は汎用 `/game <id> [見る]`**（`/blackjack`・`/bj` は別名で維持）。空/不明 id は遊べる一覧を出す。観戦/参加の人数は**登録メタ `(min,max)` にクランプ**（leduc 等2人最少のゲームを観戦 want=1 で壊さない）。UNO/leduc はアダプタ登録済み・LLM 注入/手パースはゲーム非依存でそのまま動く（カード描画は presentation 側の課題＝ADR-0019/Backlog）。
- **観戦窓は終局で自動で閉じず、手動×**（結果を見せる）。本体終了・お開き時には閉じる。
- **「ゲームモードのまま固まらない」保証**: ①観戦窓×は窓を閉じるだけでなく入力チャネル経由で scheduler に伝え、**お開き（縁側へ復帰）**にする（窓だけ閉じて `Scheduler.game` が残るのを防ぐ）②`_tick` の game ブロックは timeout 以外の例外でも**お開き**にして tick ループを殺さない ③対局中は `/codex`・`/game` をガード（room と game の同時成立を防ぐ）。終了処理は共通に集約（**2026-07-11 注記: ADR-0029 P3 で対局運用を `GameController` へ切り出し済み＝現在は `GameController._teardown`。旧称 `_teardown_game` は存在しない**）。
