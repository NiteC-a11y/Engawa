...
- [ ] **ACP 握手失敗時の teardown 漏れ＋一時dir の刈り残し（codexレビュー S2・6/29）**＝`AcpAgent.spawn()` の握手で `raise RuntimeError`（`src/acp.py:252,256`）が `except ConnectionError`（`src/acp.py:257`）に捕まらず素通り＝`shutdown_process`/task 回収なしで子プロセス・reader/stderr task が残る。`close()`（`src/acp.py:307-315`）も `_persona_dir`/guest_dir（`src/acp.py:92,279`）を rmtree しない。認証失敗の反復・来訪反復で node/npx と一時dir が積もる（常駐前提＝影響大）。直し: `spawn()` を try/except/finally で失敗時 `shutdown_process(proc)`＋task cancel を必ず走らせ、`close()` で `shutil.rmtree(self._persona_dir, ignore_errors=True)`。※上の「cmd /c の裏の node 取り残し（Job Object 化）」と同根の resource leak
- [ ] **ACP request/prompt に用途別 timeout が無く adapter 無応答で永久待ち（codexレビュー S1・6/29）**＝`ACPClient.request()`（`src/acp.py:158-163`）に timeout 無し。pending 解放は `reader()` の **EOF 時のみ**（`src/acp.py:194-196`）＝adapter が生きたまま最終 response を返さないと future が解けない。`_inject` が `turn_lock` を保持（`src/scheduler.py:66-74`）するため、cancel 後の user 入力（`src/scheduler.py:193-198`）も `turn_lock` 待ちで停止し得る。直し: `initialize`/`session/new`/`session/prompt`/`session/cancel` に用途別 timeout、timeout 時は pending を pop してプロセス再起動 or セッション終了へ倒す。※実機 claude-code-acp は cancel→`stopReason=cancelled` を返す確認済み（↑の session/cancel 項）＝正常系は緩和、他 adapter/バグには無防備なので防御として入れる
